...
const Activecard = ({
  ...
  return (
    <div
      className={` w-full px-6  flex items-center justify-around ${
    ...
    >
      ...
      <div
        className={`  flex items-start justify-center gap-4  xl:justify-between ${
          reverse ? " flex-row md:flex-row-reverse" : "flex-row"
        } w-[100%] xl:w-[50%] scale-90 xl:scale-100  `}
      >
        <div className=" flex flex-col w-[450px] gap-5 ">
          ...
          <ul className=" flex flex-col gap-2 list-[square] ">
            {list.map((item, index) => (
              <li
                key={index}
                className=" text-[16px] font-helvetica text-[#2C2C30]  "
              >
                {item}
              </li>
            ))}
